################################################################################
# Concentration-Aligned Random Forest
################################################################################

make_single_id_rot <- function(x, y) {
  y <- unlist(y)
  x <- as.matrix(x)
  ...
  same_class_indices <- which(y == class_a)
  same_class_indices <- same_class_indices[
    same_class_indices != idx_a[1]
  ]
  ...
  difference_matrix <- x[idx_a, ] - x[idx_b, ]
...
rotateDiffs_inv <- function(differences) {
...
singleDiffForest_id <- function(
  ...
  models <- lapply(
    seq_len(ntrees),
    function(i) {
      rotation_matrix <- make_single_id_rot(
        numeric_data,
        y
      )
